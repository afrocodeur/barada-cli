    ...
    removeDir: (source, keep = false) => {
        ...
        if(stat.isDirectory()){
            ...
            dirfiles.forEach((file) => {
                ...
                if(stat.isDirectory()){
                    file.removeDir(source + '/' + file);
                }
                else fs.unlinkSync(source + '/' + file);
            });
            (!keep) ? fs.unlinkSync(source) : null;
        }
    },
    copy: (source, to, filter = null, infos = {}) => {
        // statSync
        ...
        if(stat.isDirectory()){
            ...
            dirfiles.forEach((file) => {
                ...
                if(filter === null || (typeof filter === 'function' && filter(directpath, stat, infos, files))){
                    if(stat.isDirectory()){
                        let todir = to+'/'+file;
                        !fs.existsSync(todir) ? fs.mkdirSync(todir) : null;
                        files.copy(directpath, todir, filter, infos);
                    }
                    else {
